...
--*/
{
    ...
    bUnloading = FALSE;
    pWallWorkerThread = my_ex_allocate_pool( sizeof( WORKER_THREAD));
    ...
    if( !NT_SUCCESS( status ))
    {
        ...
        my_ex_free_pool( pWallWorkerThread );
        goto exit;
    }
...
--*/
{
    ...
    node = my_ex_allocate_pool( sizeof( WORKER_PROCESS_NODE ));
    if( node == NULL ){
   ...
   )
{
   ...
   if((flagsIndex != UINT_MAX) && ((in_fixed_values->incomingValue\
